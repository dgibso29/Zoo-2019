Shader "Custom/TerrainShader2"
{
	...
	#pragma require 2darray
        ...
        void surf (Input IN, inout SurfaceOutputStandard o)
        {
			...
			float4 a = heightlerp(texture1, height1, texture4, height4, uvWorld.x);
			float4 b = heightlerp(texture2, height2, texture3, height3, uvWorld.x);
			float ah = lerp(height1, height4, uvWorld.x);
			float bh = lerp(height2, height3, uvWorld.x);

			c = heightlerp(a, ah, b, bh, uvWorld.y);
